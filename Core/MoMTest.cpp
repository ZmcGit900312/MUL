...
TEST_F(MoMTest,PlanewaveTest)
{
	try
	{
		if (NotMoM)throw spd::spdlog_ex("PlanewaveTest is not Testing");
		Console->debug("Right Hand Test");
		char* excitationName = "E:/ZMC/Code/C_program/MUL/test/Excitation.txt";
		//ASSERT_EQ(0, Core::SetRightHand()) << "Error in Set RightHand";
		const size_t size = ComponentList::BFvector.size();
		VectorXcd& comp = ComponentList::ImpService->GetExcitation();
		VectorXcd ref{ size };
