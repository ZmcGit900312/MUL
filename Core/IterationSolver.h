...
namespace Core
{
	namespace Solution
	{
		...
		//Special For ArrayAIM with Jacobi
		template<>
		class BiCGSTABEMC<ImpArrayAIM, ArrayAIMJacobi> :public IterativeSolver
		{
		public:
			BiCGSTABEMC(SolverConfiguration&config) :IterativeSolver(config) {}
			int Precondition(IImpService*imp)override
			{
				ImpArrayAIM* arrayAIM = static_cast<ImpArrayAIM*>(imp);
				const clock_t start = clock();
				_bicg.compute(*arrayAIM);
				arrayAIM->LocalMatrix() = arrayAIM->LocalMatrix().inverse();				
				arrayAIM->diagflag = true;
				const clock_t end = clock();
				_preTime = double(end - start) / CLOCKS_PER_SEC;
				return 0;
			}
		...
		private:
			BiCGSTAB<ImpArrayAIM, ArrayAIMJacobi> _bicg;
		};
	}
}
